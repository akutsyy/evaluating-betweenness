/*
 * DynamicBarabasiIrisGenerator.cpp
 *
...
DynamicBarabasiIrisGenerator::DynamicBarabasiIrisGenerator(count k) : DynamicGraphSource(), k(k), degSum(0) {
	if (k <= 0) {
		throw std::runtime_error("k must be at least 1");
	}
}

void DynamicBarabasiIrisGenerator::initializeGraph() {
	if (! this->graphSet) {
...
void DynamicBarabasiIrisGenerator::generate() {
