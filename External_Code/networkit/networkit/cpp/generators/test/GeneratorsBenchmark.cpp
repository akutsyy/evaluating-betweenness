...
TEST_F(GeneratorsBenchmark, benchmarkBarabasiIrisGenerator) {
	count k = 2;
	count nMax = 100000;
	count n0 = 2;
...
TEST_F(GeneratorsBenchmark, benchBarabasiIrisGenerator2) {
	for (index i = 0; i < 10; ++i) {
		Aux::Random::setSeed(i, false);
		count n = Aux::Random::integer(100, 10000);
		count k = n / Aux::Random::integer(5, 20);
		BarabasiIrisGenerator gen(k, n, k, false);
		auto G = gen.generate();
		//EXPECT_TRUE(G.checkConsistency());
		//INFO(G.toString());
	}
