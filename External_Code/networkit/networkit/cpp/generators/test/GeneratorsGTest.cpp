...
#include "../DynamicBarabasiIrisGenerator.h"
...
#include "../BarabasiIrisGenerator.h"
...
TEST_F(GeneratorsGTest, testDynamicBarabasiIrisGeneratorSingleStep) {
	count k = 2; // number of edges added per node
	DynamicGraphSource* gen = new DynamicBarabasiIrisGenerator(k);
...
TEST_F(GeneratorsGTest, testDynamicBarabasiIrisGenerator) {
	DynamicGraphSource* gen = new DynamicBarabasiIrisGenerator(2);
...
TEST_F(GeneratorsGTest, viewDynamicBarabasiIrisGenerator) {
	DynamicGraphSource* gen = new DynamicBarabasiIrisGenerator(2);
	GraphEventProxy* Gproxy = gen->newGraph();
	Graph* G = Gproxy->G;
	gen->initializeGraph();
...
TEST_F(GeneratorsGTest, testBarabasiIrisGeneratorOriginal) {
	count k = 3;
	count nMax = 100;
	count n0 = 3;

	BarabasiIrisGenerator BarabasiIris(k, nMax, n0, false);
	Graph G = BarabasiIris.generate();
	...
	BarabasiIris = BarabasiIrisGenerator(k, nMax, initGraph, false);
	G = BarabasiIris.generate();
...
TEST_F(GeneratorsGTest, testBarabasiIrisGeneratorConstructor) {
	// k > nMax
	EXPECT_THROW(BarabasiIrisGenerator generator(10, 9, 8, false), std::runtime_error);
	EXPECT_THROW(BarabasiIrisGenerator generator(10, 9, 8, true), std::runtime_error);

	// n0 > nMax
	EXPECT_THROW(BarabasiIrisGenerator generator(5, 9, 10, false), std::runtime_error);
	EXPECT_THROW(BarabasiIrisGenerator generator(5, 9, 10, true), std::runtime_error);

	// n0 = initGraph.numberOfNodes() > nMax
	Graph initGraph(10);
	EXPECT_THROW(BarabasiIrisGenerator generator(6, 9, initGraph, false), std::runtime_error);
	EXPECT_THROW(BarabasiIrisGenerator generator(6, 9, initGraph, true), std::runtime_error);

	// initGraph, k > nMax
	initGraph = Graph(6);
	EXPECT_THROW(BarabasiIrisGenerator generator(10, 9, initGraph, false), std::runtime_error);
	EXPECT_THROW(BarabasiIrisGenerator generator(10, 9, initGraph, true), std::runtime_error);

	// initGraph, original method, initGraph.numberOfNodes() < k
	EXPECT_THROW(BarabasiIrisGenerator generator(8, 9, initGraph, false), std::runtime_error);

	// initGraph does not have consecutive node ids
	initGraph.removeNode(0);
	EXPECT_THROW(BarabasiIrisGenerator generator(3, 9, initGraph, false), std::runtime_error);
	EXPECT_THROW(BarabasiIrisGenerator generator(3, 9, initGraph, false), std::runtime_error);
}

TEST_F(GeneratorsGTest, testBarabasiIrisGeneratorBatagelj) {
	count k = 3;
	count nMax = 100;
	count n0 = 3;

	BarabasiIrisGenerator BarabasiIris(k, nMax, n0, true);
	Graph G = BarabasiIris.generate();
	...
	initGraph.addEdge(2,3);
	initGraph.addEdge(0,3);
	BarabasiIris = BarabasiIrisGenerator(k, nMax, initGraph, true);
	G = BarabasiIris.generate();
...
TEST_F(GeneratorsGTest, generatetBarabasiIrisGeneratorGraph) {
		count k = 3;
		count nMax = 1000;
		count n0 = 3;

		BarabasiIrisGenerator BarabasiIris(k, nMax, n0);
