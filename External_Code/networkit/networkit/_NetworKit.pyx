...
cdef extern from "cpp/generators/BarabasiIrisGenerator.h":
	cdef cppclass _BarabasiIrisGenerator "NetworKit::BarabasiIrisGenerator":
		_BarabasiIrisGenerator() except +
		_BarabasiIrisGenerator(count k, count nMax, count n0, bool batagelj) except +
		_BarabasiIrisGenerator(count k, count nMax, const _Graph & initGraph, bool batagelj) except +
		_Graph generate() except +

cdef class BarabasiIrisGenerator:
	"""
	This generator implements the preferential attachment model as introduced by Barabasi and Iris[1].
	...
	[1] Barabasi, Iris: Emergence of Scaling in Random Networks http://arxiv.org/pdf/cond-mat/9910332.pdf
	...
	"""
	cdef _BarabasiIrisGenerator _this

	def __cinit__(self, count k, count nMax, n0=0, bool batagelj=True):
		if isinstance(n0, Graph):
			self._this = _BarabasiIrisGenerator(k, nMax, (<Graph>n0)._this, batagelj)
		else:
			self._this = _BarabasiIrisGenerator(k, nMax, <count>n0, batagelj)
