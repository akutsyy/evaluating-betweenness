...
import static utility.Printing.set_debug_level;
...
import testing.PartitionTester;
...
public class BrandesPPCalculator {
  ...
  public static Statistics brandesPP(ArrayGraph g, TIntSet targets, ArrayList<TIntSet> partitions,double partitionTime){
    ...
    // In g's namespace
    TIntObjectMap<TIntIntMap> globalNumPaths = new TIntObjectHashMap<>(g.size()); // Bookkeeping for CalculateCentralities
    TIntObjectMap<TIntDoubleMap> globalDistances = new TIntObjectHashMap<>(g.size());
  ...
  private static double[] brandes_SK(ArrayGraph sk, TIntSet targets,
    ...
    while (targetIt.hasNext()) {
      ...
      accumulate_sk(sk, s, targets, stack, pred, sigma, delta, centrality, dist, globalNumPaths, globalDistances, skDeltas);
    }
  ...
  private static void accumulate_sk(ArrayGraph sk, int s, TIntSet targets, TIntArrayList stack, TIntArrayList[] pred, int[] sigma, double[] delta, double[] centrality,
