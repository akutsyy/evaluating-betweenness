...
/**
 * File: SchwarzFibonacciHeap.java
 *
...
public final class SchwarzFibonacciHeap<T> {
   ...
   */
  public static <T> SchwarzFibonacciHeap<T> merge(
      SchwarzFibonacciHeap<T> one, SchwarzFibonacciHeap<T> two) {
    /* Create a new FibonacciHeap to hold the result. */
    SchwarzFibonacciHeap<T> result = new SchwarzFibonacciHeap<>();
  ...
  /**
   * Utility function which, given two pointers into disjoint circularly- linked lists, merges the two lists together into one circularly-linked list in O(1) time.  Because the lists may be empty, the return value is the only pointer that's guaranteed to be to an element of the resulting list.
   *
   * This function assumes that one and two are the minimum elements of the lists they are in, and returns a pointer to whichever is smaller.  If this condition does not hold, the return value is some arbitrary pointer into the doubly-linked list.
   *
  ...
  private static <T> Entry<T> mergeLists(Entry<T> one, Entry<T> two) {
    ...
    if (one == null && two == null) { // Both null, resulting list is null.
      ...
      if (one == null && two != null) { // One is null, result is two.
        return two;
   ...
   * @throws IllegalArgumentException If the new priority exceeds the old priority, or if the argument is not a finite double.
   */
