...
public class Statistics {
  ...
  private double[] centArray = null;
  private int iterations;

  public Statistics(double[] centrality, int iterations, boolean endedFromCutoff) {
    this(centrality);
  ...
  public int getNumCentralitySubAlgorithmUses() {
  ...
  public double[] getCentralityArray() {
    if (centArray != null) {
      return centArray;
    }
    centArray = new double[centrality.size()];
    for (int i = 0; i < centArray.length; i++) {
      centArray[i] = centrality.get(i);
    }
    return centArray;
  }
