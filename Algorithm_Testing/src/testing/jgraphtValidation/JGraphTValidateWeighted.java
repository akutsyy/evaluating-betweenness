package testing.jgraphtValidation;
...
class JGraphTValidateWeighted {
  ...
  public void findConnected() {
    KosarajuStrongConnectivityInspector<Integer, DefaultWeightedEdge> connectivityInspector = new KosarajuStrongConnectivityInspector<>(jGraph);
    int largest = connectivityInspector.stronglyConnectedSets().stream().mapToInt(Set::size).max().getAsInt();
    System.out.println(connectivityInspector.stronglyConnectedSets().size() + " strongly connected sets, largest is " + largest + " nodes");
  }
}
