...
import static utility.BinarySearch.binarySearchJustLessThan;
import static utility.BinarySearch.binarySearchJustMoreThan;
...
public class GeisbergerCalculator {
          ...
          while (it.hasNext()) {
            ...
            if (sigma[w] != 0) {
              ...
              if (g.isWeighted()) {
                ...
                distV = weightedDist[v];
              } else {
                distW = unweightedDist[w];
                distV = unweightedDist[v];
              }
              delta[v] += (distV / distW) * ((double) sigma[v] / sigma[w]) * (1 + delta[w]);
            }
          }
        }
        if (w != s) {
  ...
  private static void decrementHalf(TIntArrayList currPath, double[] delta, double[] weightedDist, boolean isForwardSearch, long[] sigma) {
